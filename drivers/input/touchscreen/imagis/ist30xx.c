...
void get_fw_ver_bin(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "IM00%04x", ist30xx_parse_param_ver(PARSE_FLAG_FW, ist30xx_fw_pff));
...
void get_threshold(void *device_data)
{
	struct ist30xx_data *info = (struct ist30xx_data *)device_data;
	char buff[16] = {0};
	int v_threshold=200; //touch_key = 300
...
//Implemented the function to execute when the device attribute touchkey_threshold is call
static ssize_t touchkey_threshold_show(struct device *dev,  struct device_attribute *attr, char *buf)
{
    ...
    return snprintf(buf, sizeof(int), "%d", threshold);
}
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	tsp_fw_version= ist30xx_parse_param_ver(PARSE_FLAG_FW, ist30xx_fw_pff);
...
static struct attribute *fac_touchkey_attributes[] = {
	...
	&dev_attr_touchkey_threshold.attr,		//added for the sysfs touchkey threshold entry for Nevis VE eur
	NULL,
};
