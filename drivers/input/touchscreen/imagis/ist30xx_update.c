...
const u8 *ist30xx_fw = ist30xx_fw_pff;
const u8 *ist30xx_param = ist30xx_param_pff;
u32 ist30xx_fw_size = sizeof(ist30xx_fw_pff);
u32 ist30xx_param_size = sizeof(ist30xx_param_pff);
...
int ist30xx_isp_fw_update(struct i2c_client *client, const u8 *buf)
{
	...
	for (i = 0; i < page_cnt; i++) {
		...
		if (ret) {
			pr_err("[ TSP ] ISP cmd fail, CMD_ISP_PROGRAM_PAGE, page i:%d error\n",i);
			return ret;
		}
...
{
	int ret = 0;
	volatile int retry = IST30XX_FW_UPDATE_RETRY;
	...
	if (data->tsp_type /*ist30xx_tsp_type*/ == 0||data->tsp_type /*ist30xx_tsp_type*/ == (u32)(-1)) {    // tsp_type : 0 --> PFF
	...
	} else if (data->tsp_type /*ist30xx_tsp_type*/ == 1) { // tsp_type : 1 --> PF
		ist30xx_fw = ist30xx_fw_pf;
		ist30xx_param = ist30xx_param_pf;
		ist30xx_fw_size = sizeof(ist30xx_fw_pf);
		ist30xx_param_size = sizeof(ist30xx_param_pf);
	} else {
		return -EPERM;
	}
