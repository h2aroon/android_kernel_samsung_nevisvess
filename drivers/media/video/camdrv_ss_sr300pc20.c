...
#include "camdrv_ss.h"
#if defined(CONFIG_MACH_RHEA_SS_CORSICA)|| defined(CONFIG_MACH_RHEA_SS_NEVISW) || defined(CONFIG_MACH_RHEA_SS_NEVISNVESS)|| defined(CONFIG_MACH_RHEA_SS_NEVISVESS)
#include "camdrv_ss_sr300pc20_nevisw.h"
#else
...
int camdrv_ss_sr300pc20_set_preview_start(struct v4l2_subdev *sd)
{
	...
	if(state->mode_switch == CAMERA_PREVIEW_TO_CAMCORDER_PREVIEW)
	{
		...
		if (err < 0) {
			...
			return -EIO;
		} // Need to write fixed fps setting in here for camcorder recording and preview.
	}
...
static int camdrv_ss_sr300pc20_set_capture_start(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if (state->currentScene == SCENE_MODE_NIGHTSHOT || state->currentScene == SCENE_MODE_FIREWORKS) {
		...
		ymean = (unsigned short)data;
         ...
         // calculate ISO
		if(ymean < 0x16 &&  state->currentScene == SCENE_MODE_FIREWORKS){
		...
		else if(ymean < 0x16 &&  state->currentScene == SCENE_MODE_NIGHTSHOT){
                                         ...
                                         regs_t reg_buffer[],int num_of_regs,char *name)
{
	...
	int index = 0;
	int err = 0;
	return camdrv_ss_sr300pc20_sensor_burst_write_list(client,reg_buffer,num_of_regs,name);
...
//@HW
//Power (common)
static struct regulator *VCAM_IO_1_8_V;  //LDO_HV7
...
static int camdrv_ss_sr300pc20_sensor_power(int on)
{
	...
	VCAM_IO_1_8_V = regulator_get(NULL,"hv7");
