...
static int isFirstboot = 1; // modify for the boot occurred white screen
...
||  defined(CONFIG_MACH_RHEA_SS_NEVISW) || defined(CONFIG_MACH_RHEA_SS_CORSICA) ||  defined(CONFIG_MACH_RHEA_SS_NEVISVESS)
...
switch (state) {
	...
	//		gpio_direction_input(BACKLIGHT_GPIO);
			...
			DispSmi_ExecCmndList(dispH, &power_off_seq_ili9486[0]);
    ...
    case CTRL_SLEEP_IN:
		switch (pPanel->pwrState) {
			case STATE_SCREEN_ON:
				DispSmi_ExecCmndList(dispH,  &display_off_seq_ili9486[0]);
				DispSmi_ExecCmndList(dispH,  &sleep_in_seq_ili9486[0]);
		...
		switch (pPanel->pwrState) {
            case STATE_SLEEP:
				DispSmi_ExecCmndList ( dispH, &sleep_out_seq_ili9486[0]);
    ...
    case CTRL_SCREEN_ON:
		switch (pPanel->pwrState) {
			case STATE_SCREEN_OFF:
				DispSmi_ExecCmndList(dispH, &display_on_seq_ili9486[0]);
