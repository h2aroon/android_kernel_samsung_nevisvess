...
#include "lcd_ili9486_smi.h"
#include "lcd_hx8357d_smi.h"
...
typedef struct
{
	...
	UInt32					panelID;
	DISPCTRL_REC_T 			*display_on_seq;
	DISPCTRL_REC_T 			*display_off_seq;
	DISPCTRL_REC_T 			*sleep_in_seq;
	DISPCTRL_REC_T 			*sleep_out_seq;
	DISPCTRL_REC_T 			*power_on_seq;
	DISPCTRL_REC_T 			*power_off_seq;
} DISP_SMI_PANEL_T;
...
static UInt32 DispSmi_GetID(DISPDRV_HANDLE_T dispH, UInt32 reg );
...
static CSL_SMI_CTRL_T  DISP_SMI_SmiCtrlCfg =
{
    ...
    // Read ID timing are similar for BOE and AUO
    // TODO: Plug-In Real Timing For The Display

	{ 0, 90, 10, 360, },    //  CSL_SMI_TIMIMG_T  rdTiming;
    { 16,  30, 10, 30 , },    // CSL_SMI_TIMIMG_T  wrTiming;
    { 10,  12, 10, 30 , }, // CSL_SMI_TIMIMG_T  wrTiming_m
    TRUE,                   //  usesTE
};
...
UInt32 panelID = LCD_PANEL_ID_NOT_SET;   /* it's determined in DispSmi_GetID() */
//*****************************************************************************
//
...
#if defined(CONFIG_MACH_RHEA_SS_ZANIN) || defined(CONFIG_MACH_RHEA_SS_ZANIN_04) || defined(CONFIG_MACH_RHEA_SS_IVORY)|| defined(CONFIG_MACH_RHEA_SS_IVORY_00) || defined(CONFIG_MACH_RHEA_SS_IVORY_02) \
||  defined(CONFIG_MACH_RHEA_SS_ZANIN_REV05) || defined(CONFIG_MACH_RHEA_SS_NEVIS_REV00) || defined(CONFIG_MACH_RHEA_SS_CORSICA_REV00) || defined(CONFIG_MACH_RHEA_SS_CORSICA_REV02) || defined(CONFIG_MACH_RHEA_SS_NEVIS_REV01)\
||  defined(CONFIG_MACH_RHEA_SS_IVORY_REV03) \
||  defined(CONFIG_MACH_RHEA_SS_NEVISW) || defined(CONFIG_MACH_RHEA_SS_CORSICA) || defined(CONFIG_MACH_RHEA_SS_NEVISNVESS) || defined(CONFIG_MACH_RHEA_SS_NEVISVESS)
...
Int32 DISP_SMI_Open( DISPDRV_HANDLE_T dispH )
{
	...
	printk("[DISPDRV] height : %d, width : %d\n", pPanel->disp_info->height, pPanel->disp_info->width);
	...
	if (panelID==LCD_PANEL_ID_NOT_SET) {
		...
		{
			pPanel->power_on_seq = power_on_seq_ili9486_smi;
			pPanel->power_off_seq = power_off_seq_ili9486_smi;
			pPanel->sleep_in_seq = sleep_in_seq_ili9486_smi;
			pPanel->sleep_out_seq = sleep_out_seq_ili9486_smi;
			pPanel->display_on_seq = display_on_seq_ili9486_smi;
			pPanel->display_off_seq = display_off_seq_ili9486_smi;
		}
		else if(panelID == PANEL_AUO_HX8357D)
		{
			pPanel->power_on_seq = power_on_seq_hx8357d_smi;
			pPanel->power_off_seq = power_off_seq_hx8357d_smi;
			pPanel->sleep_in_seq = sleep_in_seq_hx8357d_smi;
			pPanel->sleep_out_seq = sleep_out_seq_hx8357d_smi;
			pPanel->display_on_seq = display_on_seq_hx8357d_smi;
			pPanel->display_off_seq = display_off_seq_hx8357d_smi;
		}
		else
		{
			printk("ID read failed\n");
		}
		pPanel->panelID = panelID;
	}
...
static UInt32 DispSmi_GetID(DISPDRV_HANDLE_T dispH, UInt32 reg )
{
	   ...
	   UInt32 id1, id2, id3;
         ...
         CSL_SMI_RdDirect( lcdDrv->cslH, FALSE, &id1 ); /* First data is dummy data*/
         CSL_SMI_RdDirect( lcdDrv->cslH, FALSE, &id1 );
         printk( "LCD ID id1 = 0x%x\n",id1);
         CSL_SMI_RdDirect( lcdDrv->cslH, FALSE, &id2 );
         printk( "LCD ID id2 = 0x%x\n",id2);
         CSL_SMI_RdDirect( lcdDrv->cslH, FALSE, &id3 );
         printk( "LCD ID id3 = 0x%x\n",id3);
				   ...
				   if(id1 && id2 && id3)
				  {
					 ...
					 return 0;
#else
				  if((id1 == 0x5B) && (id2 == 0xBC) && (id3 == 0xD0)){
					 return PANEL_BOE_ILI9486;
				  }else{
					 return PANEL_AUO_HX8357D;
				  }
#endif	
}
...
switch (state) {
	case CTRL_PWR_ON:
		switch (pPanel->pwrState) {
			case STATE_PWR_OFF:
			  ...
			  //DispSmi_ExecCmndList ( dispH, &power_on_seq_ili9486_BOE[0]);
                   ...
                   /*if(LCD_PANEL_ID_NOT_SET == panelID){
                       ...
                       /*if(LCD_PANEL_ID_NOT_SET == panelID)
                           panelID = DispSmi_GetID(dispH,0x04);*/
			 			DispSmi_ExecCmndList(dispH, pPanel->power_on_seq);
                   }
	...
	case CTRL_PWR_OFF:
		if (pPanel->pwrState != STATE_PWR_OFF) {
			...
			DispSmi_ExecCmndList(dispH, pPanel->power_off_seq);
    ...
    case CTRL_SLEEP_IN:
		switch (pPanel->pwrState) {
			case STATE_SCREEN_ON:
				DispSmi_ExecCmndList(dispH,  pPanel->display_off_seq);
				DispSmi_ExecCmndList(dispH,  pPanel->sleep_in_seq);
   ...
   case CTRL_SLEEP_OUT:
		switch (pPanel->pwrState) {
            case STATE_SLEEP:
				DispSmi_ExecCmndList ( dispH, pPanel->sleep_out_seq);
    ...
    case CTRL_SCREEN_ON:
		switch (pPanel->pwrState) {
			case STATE_SCREEN_OFF:
				DispSmi_ExecCmndList(dispH, pPanel->display_on_seq);
