...
#ifdef CONFIG_MACH_RHEA_SS_NEVISW
/*start-> [P130607-5352]reslove the restart pop noise issue, during restart processing(before kernel_restart()),
    to trun off HS amp if it is on to avoid pop noise.*/
static int preclose_hs_amp()
{
	if(bcmpmu_audio->HS_On)
	{
		bcmpmu_hs_power(0);
...
static struct notifier_block nb_reboot_hs_amp_block = {
	.notifier_call = preclose_hs_amp
};
/*[P130607-5352]reslove the restart pop noise issue->end*/
#endif
#ifdef CONFIG_MACH_RHEA_SS_NEVISVESS
/*start-> [P130607-5352]reslove the restart pop noise issue, during restart processing(before kernel_restart()),
    to trun off HS amp if it is on to avoid pop noise.*/
...
static int __devinit bcmpmu_audio_probe(struct platform_device *pdev)
{
	...
	*/
	...
	#ifdef CONFIG_MACH_RHEA_SS_NEVISW
   /*[P130607-5352]reslove the restart pop noise issue*/
		register_reboot_notifier(&nb_reboot_hs_amp_block);
    #endif
	#ifdef CONFIG_MACH_RHEA_SS_NEVISVESS
   /*[P130607-5352]reslove the restart pop noise issue*/
		register_reboot_notifier(&nb_reboot_hs_amp_block);
