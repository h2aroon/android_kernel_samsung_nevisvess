...
#if defined(CONFIG_MACH_ZANIN_CHN_OPEN) || defined(CONFIG_RTC_CHN_ALARM_BOOT)
...
struct rtc_time alarm_clock;	
int alarm_clock_set = 0;
...
static int bcmpmu_set_alarm_boot(struct device *dev, struct rtc_wkalrm *alarm)
{
	...
	alarm_clock.tm_year  = alarm->time.tm_year ;
	ret = rdata->bcmpmu->write_dev(rdata->bcmpmu, PMU_REG_RTCYR_ALM,
				alarm->time.tm_year - 100, PMU_BITMASK_ALL);
	if (unlikely(ret))
		goto err;

	alarm_clock.tm_mon = alarm->time.tm_mon ;
	ret = rdata->bcmpmu->write_dev(rdata->bcmpmu, PMU_REG_RTCMT_ALM,
				alarm->time.tm_mon + 1, PMU_BITMASK_ALL);
	if (unlikely(ret))
		goto err;

	alarm_clock.tm_mday = alarm->time.tm_mday;
	ret = rdata->bcmpmu->write_dev(rdata->bcmpmu, PMU_REG_RTCDT_ALM,
				alarm->time.tm_mday, PMU_BITMASK_ALL);
	if (unlikely(ret))
		goto err;

	alarm_clock.tm_hour = alarm->time.tm_hour;
	ret = rdata->bcmpmu->write_dev(rdata->bcmpmu, PMU_REG_RTCHR_ALM,
				alarm->time.tm_hour, PMU_BITMASK_ALL);
	if (unlikely(ret))
		goto err;

	alarm_clock.tm_min = alarm->time.tm_min;
	ret = rdata->bcmpmu->write_dev(rdata->bcmpmu, PMU_REG_RTCMN_ALM,
				alarm->time.tm_min, PMU_BITMASK_ALL);
	if (unlikely(ret))
		goto err;

	alarm_clock.tm_sec = alarm->time.tm_sec;
	ret = rdata->bcmpmu->write_dev(rdata->bcmpmu, PMU_REG_RTCSC_ALM,
				alarm->time.tm_sec, PMU_BITMASK_ALL);
	if (unlikely(ret))
		goto err;
...
static void  bcmpmu_rtc_shutdown(struct platform_device *pdev)
{
	...
	mutex_lock(&rdata->lock);
	rdata->bcmpmu->write_dev(rdata->bcmpmu, PMU_REG_RTCYR_ALM,
				alarm_clock.tm_year - 100, PMU_BITMASK_ALL);

	rdata->bcmpmu->write_dev(rdata->bcmpmu, PMU_REG_RTCMT_ALM,
				alarm_clock.tm_mon + 1, PMU_BITMASK_ALL);
	
	rdata->bcmpmu->write_dev(rdata->bcmpmu, PMU_REG_RTCDT_ALM,
				alarm_clock.tm_mday, PMU_BITMASK_ALL);

	rdata->bcmpmu->write_dev(rdata->bcmpmu, PMU_REG_RTCHR_ALM,
				alarm_clock.tm_hour, PMU_BITMASK_ALL);
	
	rdata->bcmpmu->write_dev(rdata->bcmpmu, PMU_REG_RTCMN_ALM,
				alarm_clock.tm_min, PMU_BITMASK_ALL);

	rdata->bcmpmu->write_dev(rdata->bcmpmu, PMU_REG_RTCSC_ALM,
				alarm_clock.tm_sec, PMU_BITMASK_ALL);
	mutex_unlock(&rdata->lock);
#endif	
}
