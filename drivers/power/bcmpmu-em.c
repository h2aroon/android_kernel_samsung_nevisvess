...
static int update_batt_capacity(struct bcmpmu_em *pem, int *cap)
{
	...
	/*bug for issue P130515-1822. This code to avoid Battery run from 0% to 255% */
	if(capacity < 0)
	{
		capacity = 0;
...
err:
	...
	if( pem->mode != MODE_RETRY || pem->mode != MODE_ADC) {
	//if( pem->mode <= MODE_RETRY ){
	
		if (!is_charger_present(pem)) {
			if (pem->batt_volt > pem->fg_lbat_lvl)
				pem->mode = MODE_HIGHBAT;
			else if (pem->batt_volt <= pem->fg_lbat_lvl)
	...
	printk(KERN_INFO "%s: cap = %d, new_mode=%d\n"
		, __func__ ,pem->batt_capacity ,pem->mode );
...
static int __devinit bcmpmu_em_probe(struct platform_device *pdev)
{
	...
	if (pdata->fg_tc_dn_lvl)
		...
		pem->fg_tc_dn_zone = FG_TMP_ZONE_0;
